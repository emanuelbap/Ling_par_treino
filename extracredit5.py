...
class Prepro:
    ...
    @staticmethod
    def filter(source):
        # Here, I'll use the comments remove feature and find the constants...
        source = Prepro.remove_comments(source)

        constants = {}
        new_lines = []
        # I'll traverse the lines in search of constants
        for line in source.splitlines():
            stripped = line.strip() # gasp each word

            if stripped.startswith("const "):
                # agora, vamos fazer uma filtragem de viado
                match = re.fullmatch(r"const\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*([0-9]+)\s*;", stripped)
...
class SymbolTable:
    ...
    def set(self, name, value):
        # Now, i need to check if the variable is mutable
        if name in self.table and self.table[name].mutable == False:
            raise Exception(f"[Semantic] Cannot change immutable variable {name}")
        
        self.table[name] = variable(value)
    

    # Now, I'm going to create a function for immutable viable values
    def create_immutable(self, name, value):
        if name in self.table:
            raise Exception(f"[Semantic] Variable {name} already defined")
        
        self.table[name] = variable(value, False)
...
class VarDec(Node):
    def evaluate(self, st):
        ...
        st.create_immutable(var_name, value)
...
class Parser:
    ...
    def parserStatement(self):
        ...
        if self.lexer.next.type == Token.LET:
            ...
            return VarDec("let", [Identifier_node, expression_node])
