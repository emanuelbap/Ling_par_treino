...
class Parser:
    ...
    def parserFact(self):
        result = self.parserAtom()

        while self.lexer.next.type == Token.FACT:
            self.eat(Token.FACT)
            result = FactOp('!', [result])
    ...
    def parserAtom(self):
        ...
        if self.lexer.next.type == Token.INT:
            node = IntVal(self.lexer.next.value, [])
            self.eat(Token.INT)
            return node
